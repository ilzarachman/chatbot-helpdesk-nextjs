...
export default function Sidebar() {
  ...
  return (
    <section
      className="h-svh flex-shrink-0 transition-[width] duration-300 bg-blue-50 text-slate-700"
      style={{
        width: sidebarOpen.value ? "300px" : "0px",
        visibility: sidebarTransitionContext.value
          ? "visible"
          : sidebarOpen.value
          ? "visible"
          : "hidden",
    ...
    >
      <nav id="sidebar" className="flex flex-col h-full p-3 w-[300px] pr-2">
        ...
        <Button
          className="my-4 p-3 mx-4 bg-[#3498DB] hover:bg-[#175782]"
          onClick={() => {
        ...
        <div
          id="history-sidebar"
          className="flex-col flex-1 transition-opacity duration-500 pr-2 overflow-y-auto"
        >
          <History histories={conversations} updateHistory={getConversations} />
        </div>

        <div id="notify-user w-full">
          <p className="p-3 pb-0 pt-4 text-xs leading-2 text-muted-foreground text-justify">
            Harap diperhatikan bahwa fitur ini saat ini masih dalam tahap
            percobaan. Kami sedang bekerja keras untuk melakukan perbaikan dan
            menyambut baik masukan Anda. Pengalaman dan saran Anda sangat
            berharga bagi kami dalam meningkatkan layanan ini. Terima kasih atas
