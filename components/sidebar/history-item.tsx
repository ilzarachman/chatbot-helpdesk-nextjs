...
export default function SidebarItem({ text, uuid }: { text: string; uuid: string }) {
    return (
        <Link
            href={`/${uuid}`}
            className="block group relative grow  whitespace-nowrap overflow-hidden p-2 pl-3 hover:bg-muted rounded-sm text-accent-foreground"
        >
            {text}
            <div className="absolute right-0 top-0 w-7 h-full bg-gradient-to-l from-40% from-background to-transparent group-hover:from-background group-hover:w-14"></div>
        </Link>
    );
}
