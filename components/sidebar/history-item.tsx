...
export default function SidebarItem({ text, uuid }: { text: string; uuid: string }) {
    return (
        <Link
            href={`/${uuid}`}
            className="block group relative grow  whitespace-nowrap overflow-hidden p-2 pl-3 hover:bg-blue-100 rounded-sm"
        >
            {text}
            <div className="absolute right-0 top-0 w-7 h-full bg-gradient-to-l from-40% from-blue-50 to-transparent group-hover:from-blue-50 group-hover:w-14"></div>
        </Link>
    );
}
